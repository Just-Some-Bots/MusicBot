...
class Playlist(EventEmitter):
    ...
    async def async_process_sc_bc_playlist(self, playlist_url, **meta):
        ...
        if baditems:
            print("[%s][WARNING]Skipped %s bad entries" % (datetime.datetime.now(), baditems))
    ...
    async def get_next_entry(self, player, predownload_next=True):
        """
        ...
        if player.looping == True:
            return player.last_entry
        if not self.entries:
            return None
        if player.islooped == False:
            entry = self.entries.popleft()
        else:
            if not player.last_entry:
                entry = self.entries.popleft()
            else:
                entry = player.last_entry
