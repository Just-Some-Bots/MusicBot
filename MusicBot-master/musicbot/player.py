...
class PatchedBuff:
    ...
    def read(self, frame_size):
        ...
        if self.volume != 1:
            frame = self._frame_vol(frame, self.volume, maxv=5)
    ...
    def _frame_vol(self, frame, mult, *, maxv=5, use_audioop=True):
        if use_audioop:
...
class MusicPlayerState(Enum):
    STOPPED = 0  # The player isn't playing anything
...
class MusicPlayer(EventEmitter):
    ...
    def looping(self):
        self.state = MusicPlayerState.LOOPING
    ...
    def resume(self):
        ...
        if self.is_paused and not self._current_player:
            ...
            self._kill_current_player()
            return
        raise ValueError("M-master I can't do that while I'm in state __** `%s`**__" % self.state)
    ...
    def _playback_finished(self):
        ...
        if not self.bot.config.save_videos and entry:
            if any([entry.filename == e.filename for e in self.playlist.entries]):
                print("[%s][INFO] Skipping deletion, found song in queue" % datetime.datetime.now())
            elif self.looping == False:
                print("[%s][INFO] Deleting file: %s" % (datetime.datetime.now(), os.path.relpath(entry.filename)))
                asyncio.ensure_future(self._delete_file(entry.filename))
            else:
                print("[%s][INFO] Skipping deletion, song on loop" % datetime.datetime.now())
    ...
    async def _play(self, _continue=False):
        """
            Plays the next entry from the playlist, or resumes playback of the current entry if paused, or restarts the song if it's looping.
        """
        if self.is_paused:
            return self.resume()
        if self.is_dead:
        ...
        with await self._play_lock:
            if self.is_stopped or _continue:
                try:
                    entry = await self.playlist.get_next_entry(self)   # PAY ATTENTION! THIS IS WHERE THE LOOPING MAGIC WILL BE WHEN IT'S TIME!
                    #if self.looping == True:
                    #    print("[%s][INFO] Was first entry, parsing loop once to access..." % datetime.datetime.now())
                    #    entry = self._current_entry
                except Exception as e:
                    print("[%s][WARNING] Failed to get entry." % datetime.datetime.now())
                    traceback.print_exc()
    ...
    async def websocket_check(self):
        if self.bot.config.debug_mode:
            print("[%s][INFO] Creating websocket check loop" % datetime.datetime.now())
        while not self.is_dead:
            try:
                ...
                if self.bot.config.debug_mode:
                    print("[%s][INFO] Voice websocket is %s, reconnecting" % (datetime.datetime.now(), self.voice_client.ws.state_name))
