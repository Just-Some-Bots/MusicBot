...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None, aliases_file=None):
        ...
        self.botstr = Json(self.config.i18nbot_file)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.members and author_perms.skip_when_absent:
                newmsg = ('on_player_play-onChannel_authorNotInChannel_skipWhenAbsent', "Skipping next song in {channel}: {title} added by {author} as queuer not in voice!").format (
                    channel=player.voice_client.channel.name,
                    title=entry.title,
                    author=entry.meta['author'].name)
                player.skip()
            elif self.config.now_playing_mentions:
                newmsg = self.botstr.get('on_player_play-onChannel_plaingMention', "{author} - your song {title} is now playing in {channel}!").format (
                    author= entry.meta['author'].mention,
                    title= entry.title,
                    channel= player.voice_client.channel.name
                )
            else:
                newmsg = self.botstr.get('on_player_play-onChannel', "Now playing in {channel}: {title} added by {author}!").format(
                    channel=player.voice_client.channel.name,
                    title=entry.title,
                    author=entry.meta['author'].name
                )
            ...
            newmsg = self.botstr.get ('on_player_play-onChannel_noAuthor_autoplalist', "Now playing automatically added entry {title} in {channel}!").format (
                title=entry.title,
                channel=player.voice_client.channel.name
                )
