...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        if not self.config.footer_text:
            text = 'Just-Some-Bots/MusicBot ({})'.format(BOTVERSION)
        else:
            text = '{}'.format(self.config.footer_text)
        e = discord.Embed()
        e.colour = 7506394
        if not self.config.disableembedfooter:
            e.set_footer(text=text, icon_url='https://i.imgur.com/gFHBoZA.png')
