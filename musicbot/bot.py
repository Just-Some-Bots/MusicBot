...
class MusicBot(discord.Client):
    ...
    async def cmd_remove(self, id, player):
        """
        Usage:
            {command_prefix}remove [song_id]

        Remove a song in queue.
        """

        id = int(id)
        if (id > 0):
            songAmt = len(player.playlist.entries)
            if (songAmt > 0):

                #id exists in song list
                if (id <= songAmt):
                    song = player.playlist.remove_entry(id - 1)
                    return Response(":sun_with_face: :dagger: Removing id: " + str(id) + "/" + song.title, delete_after=20)

    
    async def cmd_promote(self, id, player):
        """
        Usage:
            {command_prefix}promote [song_id]

        Moves a song in queue to the first.
        """
    ...
    async def cmd_zibin(self, player):
        """
        Usage:
            {command_prefix}zibin

        OMEGALUL.
        """

        player.increase_play_count()
        return Response("O M E G A L U L", 10)

    async def cmd_playcount(self, player, id):
        """
        Usage:
            {command_prefix}playcount [current|song_id_from_youtube]

        Checks how many times a song has been played.
        """
    ...
    async def cmd_requeuesong(self, player):
        """
        Usage:
            {command_prefix}requeuesong

        Adds the current playing song to queue.
        """
		
        if player.current_entry:
            #audio_cache\youtube-12345678912-TITLE.EXTENSION
            songURL = "https://www.youtube.com/watch?v=" + player.current_entry.url[20:31]
            await player.playlist.add_entry(songURL)
