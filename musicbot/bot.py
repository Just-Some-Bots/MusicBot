...
class MusicBot(discord.Client):
    ...
    async def cmd_promote(self, player, position=None):
        ...
        if length < 2:
            raise exceptions.CommandError(
                self.str.get(
                    "cmd-promote-not-enough-entries",
        ...
        if not position:
            ...
            try:
                ...
                raise exceptions.CommandError(
                    self.str.get(
                        "cmd-promote-invalid-song-number",
                        "This is not a valid song number! Please choose a song \
                    number between 2 and %s!",
                    )
                    % length,
                    expire_in=20,
                )
            ...
            if position == 1:
                raise exceptions.CommandError(
                    self.str.get(
                        "cmd-promote-entry-already-at-top",
            ...
            if position < 1 or position > length:
                raise exceptions.CommandError(
                    self.str.get(
                        "cmd-promote-invalid-song-number-2",
        ...
        reply_text = self.str.get(
            "cmd-promote-successful",
            "Promoted **%s** to the :top: of the queue. Estimated time until playing: %s",
        )
        btext = entry.title
    ...
    async def cmd_playnow(
        ...
        if not info:
            raise exceptions.CommandError(
                self.str.get(
                    "cmd-playnow-unplayable-entry", "That video cannot be played."
                ),
                expire_in=30,
            )
        ...
        if "entries" in info:
            raise exceptions.CommandError(
                self.str.get(
                    "cmd-playnow-playlist-not-allowed",
            ...
            try:
                ...
                if self.config.embeds:
                    ...
                    content.add_field(
                        ...
                        value="{}".format(
                            self.str.get(
                                "cmd-playnow-succesful",
                                "Enqueued **%s** to be played. Position in queue: Up next!",
                            )
                            % entry.title
                        ),
                        inline=True,
