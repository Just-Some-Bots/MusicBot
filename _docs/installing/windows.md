...
> If `git clone` shows error `fetal: unable to access 'https://github.com/Just-Some-Bots/MusicBot.git' SSL certificate problem: self signed certificate in certificate chain` then try disabling your antivirus. Some antivirus (such as Kaspersky Internet Security) is known to interfere with git.
